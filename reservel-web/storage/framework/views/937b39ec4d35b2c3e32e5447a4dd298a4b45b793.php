...
<main>
	<div class="wrapper">
		...
		<?php endif; ?>
		<section>
			<form action="<?php echo e(route('reserve.confirm')); ?>" method="POST">
				<?php echo e(csrf_field()); ?>

				<div class="type"><span>受付区分</span>：<span><?php echo e($careTypeName['name']); ?></span></div>
				<input type="hidden" name="careType" value="<?php echo e($careType); ?>" />
				<dl class="form_items">
					<?php if($careType==config('const.CARE_TYPE.REGULAR') || $careType==config('const.CARE_TYPE.ETC')): ?>
					...
					<dd><input type="text" id="name" name="name" value="<?php echo e(old('name')); ?>" placeholder="例）医療　太郎" required /></dd>
					<dt class="required"><label for="age">年齢</label></dt>
					<dd><input type="text" id="age" name="age" style="width:5rem" svalue="<?php echo e(old('age')); ?>" required /></dd>
					<dt class="required"><label for="purpose">性別</label></dt>
					<dd>
					<?php $__currentLoopData = config('const.GENDER'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $genderKey => $gender): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<label class="form_item_group">
							<input type="radio" id="gender_<?php echo e($genderKey); ?>" name="gender" value="<?php echo e($genderKey); ?>" <?php echo e(old("gender") == $genderKey ? 'checked="checked"' : ''); ?>/><?php echo e($gender); ?>

						</label>
						<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
					</dd>
					<dt class="required"><label for="email">メールアドレス</label></dt>
					<dd><input type="email" id="email" name="email" value="<?php echo e(old('email')); ?>" placeholder="例）taro.medical@gmail.com" required /></dd>
					<dt class="required"><label for="tel">電話番号</label></dt>
					<dd><input type="tel" id="tel" name="tel" value="<?php echo e(old('tel')); ?>" placeholder="例）0331234567" required /></dd>
				...
				<div class="privacy_attention">
					...
					<p class="privacy_attention_prompt">上記事項をご確認の上、ご同意いただける方は下の「同意して次へ」をクリックしてください。</p>
						 <div class="console">
							 <a href="<?php echo e(route('index')); ?>" class="btn_cancel" accesskey="c">キャンセル</a>
...
$(function(){
	...
	$('#btn_execution').click(function(ev){
			if ($('#savable').prop('checked')){
					...
					<?php $__currentLoopData = config('const.PET_TYPE_NAME'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $petKey => $petName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							localStorage.setItem('reserve.pet_type_' + <?php echo $petKey; ?>, document.getElementById("pet_type_" + <?php echo $petKey; ?>).checked);
					<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
					<?php $__currentLoopData = config('const.PURPOSE'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $purposeKey => $purposeName): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.common', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/itc-dev/laravel_project/Clinic_Reservel_demo/reservel-web/resources/views/reserve/create.blade.php ENDPATH**/ ?>